    ...
    if (isset($_POST['book-room-btn'])) {
        ...
        do {
            ...
            if ($conn->query($sqlInsert)) {
                ...
                $booking_link  = "kulso_foglalas.php?id=" . $reservationId; 

                $checkin  = new DateTime($checkinDate);
                $checkout = new DateTime($checkoutDate);
                $diff = $checkin->diff($checkout);
                $nights_count = $diff->days;
                $orig_price_per_night = $room['price_per_night'];
                $discountPercent = 5; 
                $discountMultiplier = (100 - $discountPercent) / 100.0;

                $price_per_night = $orig_price_per_night * $discountMultiplier;
                $total_price = $nights_count * $price_per_night;

                $subject = "Foglalás visszaigazolása (kedvezményes) - " . $hotel_name;
                ...
                <body style='font-family: Arial, sans-serif; background-color:#f5f5f5; margin:0; padding:20px;'>
                    <div style='max-width:600px; margin:0 auto; background:#ffffff; border:1px solid #dddddd; border-radius:4px; padding:20px;'>
                        ...
                        <p>Köszönjük, hogy partner oldalunkon keresztül foglaltál a(z) <strong>{$hotel_name}</strong> szállodába. Az alábbiakban összefoglaltuk a foglalásod adatait.</p>
                        <table style='width:100%; border-collapse:collapse; margin:15px 0;' role='presentation'>
                            ...
                            <tr><th align='left'>Kedvezményes ár / éj</th><td>". $price_per_night ." Ft</td></tr>
                ...
                $message = "Foglalásod rögzítettük kedvezményes áron! Ellenőrizd az emailjeidet is.";
            }
...
<body>
    <div class="page">
        <div class="wrapper">
            <a class="btn" href="index.php">← Vissza a kedvezményes szobákhoz</a>
            <?php if ($typeData): ?>
                <h1>
                    <?= $typeData['name'] ?>
                    (<?= (int)$typeData['capacity'] ?> fő) – Külső partner oldal
                </h1>
                <p>
                    Az itt szereplő árak a hotel listaárához képest
                    <strong><?= (int)$discountPercent ?>%</strong>-kal kedvezőbbek.
                </p>
                <?php if (!empty($typeData['description'])): ?>
                    <p><?= $typeData['description'] ?></p>
                <?php endif; ?>
            <?php else: ?>
                <h1>Külső foglaló – Szobatípus</h1>
                <p>Nem található ilyen szobatípus.</p>
            <?php endif; ?>
            <?php if ($typeData): ?>
                <?php if (count($rooms) > 0): ?>
                        <h2>Elérhető szobák ebből a típusból</h2>
                        <small>Tipp: dátumoknál a rendszer úgyis szól, ha ütközik.</small>
                    <table>
                        <tr>
                            <th>Szobaszám</th>
                            <th>Emelet</th>
                            <th>Hotel ár / éj</th>
                            <th>Kedvezményes ár / éj</th>
                            <th>Foglalás</th>
                        </tr>
                        <?php foreach ($rooms as $r): ?>
                            <?php
                            $origPrice = (float)$r['price_per_night'];
                            $discPrice = $origPrice * $discountMultiplier;
                            ?>
                            <tr>
                                <td><strong><?= htmlspecialchars($r['room_number']) ?></strong></td>
                                <td><?= (int)$r['floor'] ?></td>
                                <td><?= (int)$origPrice ?> Ft/éjszaka</td>
                                ...
                                <td>
                                    <form method="POST">
                                        ...
                                        <input type="submit" name="book-room-btn" value="Foglalás kedvezményesen">
                                    </form>
                                </td>
                            </tr>
                        <?php endforeach; ?>
